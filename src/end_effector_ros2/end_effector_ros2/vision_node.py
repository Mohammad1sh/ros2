...
class VisionNode(Node):
    ...
    def __init__(self):
        ...
        # Ağ kamerası (Windows köprüsü) — CAMERA_URL ayarlıysa ağ akışı okunur.
        self.camera_url     = os.environ.get('CAMERA_URL', '').strip()
        ...
        # Ağ kamerası: reader thread + en-son-kare (gecikme birikmesin)
        self._processing   = False
        self._net_frame    = None
        ...
        if self.use_gazebo_cam:
            ...
            if self.camera_url:
                self.get_logger().info(
                    f'Ağ kamerası modu — CAMERA_URL={self.camera_url} fps:{self.stream_fps}')
                threading.Thread(target=self._net_reader, daemon=True).start()
            else:
                self.get_logger().info(
    ...
    # ── Ağ kamerası okuyucu thread'i ───────────────────────────────────────────
    def _net_reader(self):
        """Ağ akışından sürekli okur, sadece EN SON kareyi tutar (gecikme birikmez)."""
        while self._running:
            ...
            if not (self.cap and self.cap.isOpened()):
                self.get_logger().info(f'Ağ kamerası açılıyor: {self.camera_url}')
                ...
                if cap.isOpened():
                    ...
                    self.get_logger().info('Ağ kamerası açıldı ✓')
                else:
            ...
            if not ret:
                self.get_logger().warn('Ağ kamerası frame yok — yeniden bağlanılıyor')
                try: self.cap.release()
                except Exception: pass
                self.cap = None
                self.pub_cam_status.publish(Bool(data=False))
    ...
    def _process_frame(self):
        ...
        # Ağ kamerası: reader thread'in tuttuğu en son kareyi işle
        if self.camera_url:
            if self._processing:
                return
            with self._net_lock:
                frame = self._net_frame.copy() if self._net_frame is not None else None
            if frame is None:
                return
    ...
    def _detect_full(self, frame: np.ndarray):
        results = self.model(np.ascontiguousarray(frame), conf=CONF_THRESHOLD, verbose=False)
        boxes = []
    ...
    def _detect_tiled(self, frame: np.ndarray):
        ...
        for y0 in ys:
            for x0 in xs:
                ...
                results = self.model(np.ascontiguousarray(tile), conf=CONF_THRESHOLD, verbose=False)
                for result in results:
                    for box in result.boxes:
